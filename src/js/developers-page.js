const DEV_API = {
   list: "https://api.erzen.xyz/oauth/applications/dev",
   register: "https://api.erzen.xyz/oauth/applications/register",
   edit: "https://api.erzen.xyz/oauth/applications/edit",
   rotate: "https://api.erzen.xyz/oauth/applications/rotate-secret",
};
   ...
   container.innerHTML = apps
      .map(
        ...
        <div class="app-card">
            ...
            <div class="app-info">
                <h4>${app.name}</h4>
                <p>Client ID: ${app.clientId} • Created ${formatTimeAgo(app.createdAt)}</p>
            </div>
...
function showEditModal(clientId) {
   ...
   modal.innerHTML = `
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editAppForm">
                    <input type="hidden" id="editClientId" value="${app.clientId}">
                    <div class="form-group">
                        <label for="editName">Application Name</label>
                        <input type="text" id="editName" value="${app.name}" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" id="editTerms" value="${app.termsOfServiceUrl || ""}">
                    </div>
                    <div class="form-group">
                        <label for="editLogoURL">Application Logo URL</label>
                        <input type="url" id="editLogoURL" value="${app.logoUrl || ""}">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
async function rotateSecret(clientId, currentSecret) {
   try {
      ...
      const data = await response.json();
      applications = applications.map((app) =>
         app.clientId === clientId ? { ...app, clientSecret: data.clientSecret } : app
      );
