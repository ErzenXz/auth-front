...
const API = {
   login: "https://api.erzen.xyz/v1/auth/login",
   register: "https://api.erzen.xyz/v1/auth/register",
   mfaSetup: "https://api.erzen.xyz/v1/auth/mfa/setup",
   profile: "https://api.erzen.xyz/v1/auth/info",
   security: "https://api.erzen.xyz/v1/user/active-sessions",
   mfaVerify: "https://api.erzen.xyz/v1/auth/mfa/verify",
   mfaSetupSecond: "https://api.erzen.xyz/v1/auth/mfa/setup/verify",
   refresh: "https://api.erzen.xyz/v1/auth/refresh",
   logout: "https://api.erzen.xyz/v1/auth/logout",
   password: "https://api.erzen.xyz/v1/auth/change-password",
};
...
function switchForm(form) {
   ...
   document.getElementById("dashboard").style.display = form === "dashboard" ? "flex" : "none";
...
async function loadUserData() {
   try {
      ...
      if (response.ok) {
         ...
         document.getElementById("lastLogin").textContent = formatRelativeTime(data.lastLogin);
      ...
      <div class="modal-content">
          ...
          <div class="modal-body">
              <div class="detail-group">
                  ...
                  <span class="detail-value">${parseUserAgent(device.userAgent)}</span>
              </div>
              ...
              <div class="detail-group">
                  ...
                  <span class="detail-value">${new Date(device.created).toLocaleString()}</span>
              </div>

              <div class="detail-group">
                  <span class="detail-label">Expires</span>
                  <span class="detail-value">${new Date(device.expires).toLocaleString()}</span>
              </div>

              <div class="detail-group">
                  <span class="detail-label">Last Used</span>
                  <span class="detail-value">${
                     device.lastUsed ? new Date(device.lastUsed).toLocaleString() : "N/A"
                  }</span>
              </div>
                 ...
                 device.revoked
                  ...
                  <div style="margin-top: 1.5rem; padding: 1rem; background: #fef2f2; border-radius: 8px;">
                      <div class="detail-group" style="margin-bottom: 0.5rem;">
                          ...
                          <span class="detail-value">${device.revokedByIp}</span>
                      </div>
                      <div class="detail-group" style="margin-bottom: 0;">
                          <span class="detail-label">Reason</span>
                          <span class="detail-value">${device.revocationReason || "N/A"}</span>
                      </div>
                  </div>
              `
                    : ""
...
async function handleRegister(event) {
   ...
   try {
      const formData = {
         ...
         birthdate: new Date(document.getElementById("registerBirthdate").value).toISOString(),
...
async function loadConnectedApps() {
   try {
      const response = await fetch("https://api.erzen.xyz/oauth/applications", {
         credentials: "include",
         headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
...
function displayConnectedApps(apps) {
   ...
   apps.forEach((app) => {
    ...
    ${app.grantedScopes.map((scope) => `<span class="permission-badge">${scope}</span>`).join("")}
      </div>
      </div>
      ...
      <button class="btn btn-danger" onclick="revokeAccess('${app.clientId}', '${
         app.applicationName
...
async function revokeAccess(clientId, appName) {
   ...
   try {
      const response = await fetch("https://api.erzen.xyz/oauth/revoke", {
         method: "POST",
         credentials: "include",
         headers: {
            "Content-Type": "application/json",
...
function viewAppDetails(clientId) {
  ...
  <i class="fas fa-times"></i>
          ...
          <span class="detail-value">${app.description || "No description provided"}</span>
          </div>
          
          <div class="detail-group">
          <span class="detail-label">Permissions</span>
          <div class="scope-list">
          ${app.grantedScopes.map((scope) => `<span class="scope-badge">${scope}</span>`).join("")}
            </div>
            </div>
...
document.addEventListener("DOMContentLoaded", async () => {
   ...
   if (loggedIn) {
      ...
      document.querySelector(".create-app").addEventListener("click", showCreateModal);
...
document.getElementById("updateProfilePhoto").addEventListener("change", async (event) => {
   ...
   try {
      const response = await fetch("https://api.erzen.xyz/storage/upload", {
         // Updated endpoint
         method: "POST",
         headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
...
async function updateProfileImage(url) {
   try {
      const response = await fetch("https://api.erzen.xyz/v1/user/change-profilePicture", {
         method: "PATCH",
         credentials: "include",
         headers: {
            "Content-Type": "application/json",
